...
class TestIPGroup(BaseGroupTest):
    """IP分组测试类"""
    ...
    def setup_page(self, logged_in_page):
        ...
        self.group_page = self.sta_group_page.ip_group

        # 导航到IP分组页面
        if not self.group_page.navigate_to_ip_group_page():
    ...
    def test_add_valid_groups(self, test_case):
        """测试添加有效的IP分组"""
        # 获取测试数据
        test_data = next((item for item in self.sta_group_data["ip_groups"]
        ...
        try:
            # 执行添加分组操作
            assert self.group_page.add_group_ip(
                test_data["name"],
            ...
            # 验证分组
            ip_list_str = "\n".join(test_data["ip_list"])
            assert self.group_page.verify_ip_group_added(test_data["name"], ip_list_str)
    ...
    def test_add_invalid_groups(self, test_case):
        ...
        try:
            ...
            # 确保在分组列表页面（回退操作）
            if not self.sta_group_page.click_text_filter(self.group_page.ip_group_link):
                pytest.fail("无法返回到IP分组列表页面")
            time.sleep(1)
            ...
            self.sta_group_page.input_text(self.group_page.group_list_input, ip_text)
    ...
    def test_search_group(self, test_case):
        ...
        try:
            ...
            # 返回到IP列表
            self.sta_group_page.click_text_filter(self.group_page.ip_group_link)

            # 执行搜索操作
            assert self.group_page.search_ip_group(search_name), "搜索操作失败"
    ...
    def test_edit_group(self, test_case):
        ...
        try:
            ...
            # 执行编辑操作
            assert self.group_page.edit_group_ip(original_name, new_name, new_ip_str)
            ...
            # 搜索并验证新分组
            assert self.group_page.search_ip_group(new_name)
            assert self.group_page.verify_ip_group_added(new_name, new_ip_str)

            # 验证原始分组不存在
            assert self.group_page.search_ip_group(original_name)
    ...
    def test_delete_group(self, test_case):
        ...
        try:
            ...
            # 执行删除操作
            assert self.group_page.delete_group_ip(group_name)

            # 验证删除结果
            assert self.group_page.search_ip_group(group_name)
            assert self.group_page.verify_no_data_prompt()
    ...
    def test_delete_all_groups(self, test_case):
        ...
        try:
            ...
            # 验证删除结果
            assert self.group_page.delete_all_ip_groups()
            assert self.group_page.verify_no_data_prompt()
...
class TestMACGroup(BaseGroupTest):
    """MAC分组测试类"""

    @pytest.fixture(scope="function", autouse=True)
    def setup_page(self, logged_in_page):
        """函数级别fixture - 导航到MAC分组页面"""
        self.sta_group_page = StaGroupPage(logged_in_page)
        self.group_page = self.sta_group_page.mac_group

        # 导航到MAC分组页面
        if not self.group_page.navigate_to_mac_group_page():
            pytest.fail("无法导航到MAC分组页面")
        yield
    ...
    def test_add_valid_groups(self, test_case):
        """测试添加有效的MAC分组"""
        # 获取测试数据
        test_data = next((item for item in self.sta_group_data["mac_groups"]
                          if item["test_case"] == test_case), None)
        if not test_data:
            pytest.skip(f"未找到测试数据: {test_case}")
        ...
        try:
            # 执行添加分组操作
            assert self.group_page.add_group_mac(
                test_data["name"],
                "\n".join(test_data["mac_list"]),
                navigate=False
            )

            # 验证分组
            mac_list_str = "\n".join(test_data["mac_list"])
            assert self.group_page.verify_mac_group_added(test_data["name"], mac_list_str)
    ...
    def test_add_invalid_groups(self, test_case):
        ...
        try:
            ...
            # 确保在分组列表页面（回退操作）
            if not self.sta_group_page.click_text_filter(self.group_page.mac_group_link):
                pytest.fail("无法返回到MAC分组列表页面")
            time.sleep(1)
            ...
            mac_list = test_data.get("mac_list", [])

            # 输入分组名称
            self.sta_group_page.input_text(self.group_page.group_name_input, actual_name)

            # 输入MAC列表
            mac_text = "\n".join(mac_list) if mac_list else ""
            self.sta_group_page.input_text(self.group_page.group_list_input, mac_text)
    ...
    def test_search_group(self, test_case):
        ...
        try:
            ...
            # 返回到MAC列表
            self.sta_group_page.click_text_filter(self.group_page.mac_group_link)

            # 执行搜索操作
            assert self.group_page.search_mac_group(search_name), "搜索操作失败"
    ...
    def test_edit_group(self, test_case):
        ...
        try:
            ...
            original_name = test_data["original_name"]
            new_name = test_data["new_name"]
            new_mac_str = "\n".join(test_data["new_mac_list"])

            # 执行编辑操作
            assert self.group_page.edit_group_mac(original_name, new_name, new_mac_str)
            ...
            # 搜索并验证新分组
            assert self.group_page.search_mac_group(new_name)
            assert self.group_page.verify_mac_group_added(new_name, new_mac_str)

            # 验证原始分组不存在
            assert self.group_page.search_mac_group(original_name)
            assert self.group_page.verify_no_data_prompt()
    ...
    def test_delete_group(self, test_case):
        ...
        try:
            ...
            # 执行删除操作
            assert self.group_page.delete_group_mac(group_name)

            # 验证删除结果
            assert self.group_page.search_mac_group(group_name)
            assert self.group_page.verify_no_data_prompt()
    ...
    def test_delete_all_groups(self, test_case):
        ...
        try:
            ...
            # 验证删除结果
            assert self.group_page.delete_all_mac_groups()
            assert self.group_page.verify_no_data_prompt()
